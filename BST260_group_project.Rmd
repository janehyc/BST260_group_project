...
logistic_fit <- glm(burn~X+Y+FFMC+I(FFMC^2)+DMC+I(DMC^2)+DC+I(DC^2)+ISI+temp+I(temp^2)+RH+wind+rain+I(rain^2), train_set, family=binomial(link = "logit"))
...
knn_fit <- knn3(burn~X+Y+FFMC+I(FFMC^2)+DMC+I(DMC^2)+DC+I(DC^2)+ISI+temp+I(temp^2)+RH+wind+rain+I(rain^2), data=train_set,k=19)
...
res <- train(burn~ X+Y+FFMC+I(FFMC^2)+DMC+I(DMC^2)+DC+I(DC^2)+ISI+temp+I(temp^2)+RH+wind+rain+I(rain^2), data=dat2, method="knn", trControl=control,
             tuneLength=1, tuneGrid=data.frame(k=seq(1,100,2)), metric="Accuracy")
