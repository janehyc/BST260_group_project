...
table1<- matrix(c(summary(mod_lin)$r.squared, summary(mod_lin)$adj.r.squared, sqrt(mean(mod_lin$residuals^2)),AIC(mod_lin), BIC(mod_lin),
                  ...
                  summary(step.model)$r.squared, summary(step.model)$adj.r.squared, sqrt(mean(step.model$residuals^2)),AIC(step.model), BIC(step.model)), ncol=5,nrow=3, byrow=TRUE)
colnames(table1)<- c("R^2", "Adjusted R^2", "Square root of MSE","AIC", "BIC")
rownames(table1)<- c("logarea ~.",
                     "logarea~., (w/o influential points)",
                     "model with backward selection based on AIC values")
...
hoslem.test(new_dat$burn, fitted(lg_burn), g=10)  #not a poor fit (H0: small P-value, poor fit)
