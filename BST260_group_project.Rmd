...
logistic_fit <- glm(burn~X+Y+FFMC+DMC+DC+ISI+temp+RH+wind+rain, train_set, family=binomial(link = "logit"))
p_hat_logit <- predict(logistic_fit, newdata=test_set)
y_hat_logit <- ifelse(p_hat_logit>0.5, 1,0)
confusionMatrix(data=as.factor(y_hat_logit), reference = as.factor(test_set$burn))  #0.528

#knn
f_hat <- predict(logistic_fit, newdata=test_set, type="response")
tab <- table(pred=round(f_hat), truth=test_set$burn)
confusionMatrix(tab)$tab
confusionMatrix(tab)$overall["Accuracy"]
knn_fit <- knn3(burn~X+Y+FFMC+DMC+DC+ISI+temp+RH+wind+rain, data=train_set,k=7)
...
res <- train(burn~ X+Y+FFMC+DMC+DC+ISI+temp+RH+wind+rain, data=dat2, method="knn", trControl=control,
             tuneLength=1, tuneGrid=data.frame(k=seq(1,152,2)), metric="Accuracy")
plot(res)


# Gamma
library(e1071)
ols_fit <- lm(area~ X+Y+FFMC+DMC+DC+ISI+temp+RH+wind+rain, train_set[train_set$area>0,])
gamma_fit <- glm(area_1~X+Y+FFMC+DMC+DC+ISI+temp+RH+wind+rain, train_set,family = Gamma(link = 'log'))

#nb_fit   <- naiveBayes(area_1~X+Y+FFMC+DMC+DC+ISI+temp+RH+wind+rain, data = train_set)
#y_hat_nb <- predict(nb_fit, test_set)
#confusionMatrix(factor(y_hat_nb, levels=1:148), factor(test_set$area_1, levels=1:148))
...
gamma.shape(gamma_fit, verbose=TRUE)  #a=0.59031959
y.gamma <- d* rgamma(n, shape=0.59031959, rate=0.04409466)
y <- pred$burn
